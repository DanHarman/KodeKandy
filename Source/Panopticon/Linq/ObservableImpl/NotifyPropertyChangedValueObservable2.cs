...
namespace KodeKandy.Panopticon.Linq.ObservableImpl
{
        ...
        where TClass : class, INotifyPropertyChanged
    {
        ...
        private void OnPropertyChanged(object sender, PropertyChangedEventArgs propertyChangedEventArgs)
        {
            if (_source == sender && _propertyName == propertyChangedEventArgs.PropertyName)
            {
                ...
                lock (_gate)
                {
                    ...
                    _propertyValueChanged = new PropertyValueChanged<TProperty>(_source, propertyChangedEventArgs, _propertyValueGetter(_source));
                }

                if (currObserver != NopObserver<PropertyValueChanged<TProperty>>.Instance)
                    currObserver.OnNext(_propertyValueChanged);
            }
        }
        ...
        private class Subscription : IDisposable
        {
            ...
            private NotifyPropertyChangedValueObservable2<TClass, TProperty> _valueObservable;

            public Subscription(NotifyPropertyChangedValueObservable2<TClass, TProperty> valueObservable, IObserver<PropertyValueChanged<TProperty>> observer)
            {
